...
void enqueue (Transaksi* q, int& count, Transaksi data){
    if (count >= MAX){
        cout << "Antrian Sudah Penuh, Silahkan Tunggu\n";
        return;
    }
    *(q + count) = data;
    count++;
...
Transaksi dequeue (Transaksi* q, int& count){
    if (count == 0){
        cout << "Antrian Sekarang Sudah Kosong\n";
        return {"", "", "", "", 0};
    }
    Transaksi proses = q[0];
...
void tampilkanAntrian (Transaksi* q, int count){
    if (count == 0){
    ...
    for (int i = 0; i < count; i++) {
        string rute = (q + i)->asal + " -> " + (q + i)->tujuan;
        cout << "|" << left << setw(3) << i + 1 << " | ";
        cout << left << setw(20) << (q + i)->namaPenumpang << " | ";
        cout << left << setw(22) << (q + i)->namaKereta << " | ";
        cout << left << setw(28) << rute << " |\n";
    }
    cout << "+----+----------------------+------------------------+------------------------------+\n";
}

void push (Transaksi* s, int& top, Transaksi data){
    if (top >= MAX - 1){
        cout << "Stack Sudah Penuh, Tidak Bisa Menambahkan Data\n";
        return;
    }
    top++;
    *(s + top) = data;
...
void tampilRiwayatTransaksi (Transaksi* s, int top){
    if (top < 0){
    ...
    for (int i = top; i >= 0; i--) {
        ...
        cout << left << setw(20) << (s + i)->namaPenumpang << " | ";
        cout << left << setw(22) << (s + i)->namaKereta << " | ";
        cout << left << setw(28) << (s + i)->harga << " |\n";
    }
    cout << "+----+----------------------+------------------------+------------------------------+\n";
}

void peekData (Transaksi* s, int sTop, Transaksi* q, int qCount){
    cout << "================================================================================\n";
    cout << "|                                   STATUS PEEK                                 |\n";
    cout << "================================================================================\n";
    if(qCount > 0){
        cout << "Depan Antrian   :" << q->namaPenumpang << " (" << q->namaKereta << ")\n";
    }
    else {
        cout << "Depan Antrian   : Tidak Ada Antrian\n";
    }
    if(sTop >= 0){
        cout << "Transaksi Terakhir      :" << (s + sTop)->namaPenumpang << " (" << (s + sTop)->namaKereta << ")\n";
    }
    else {
...
int main (){
    ...
    while (true){
        ...
        if (pilihan == "1"){
            ...
            enqueue(antrian, qCount, t);
            cout << namaPenumpang << " telah masuk ke dalam antrian pembelian tiket\n";
            tampilkanAntrian(antrian, qCount);
        }
        else if (pilihan == "8"){
            Transaksi proses = dequeue(antrian, qCount);
            if (proses.namaPenumpang != ""){
                push(riwayat, sTop, proses);
            ...
            tampilRiwayatTransaksi(riwayat, sTop);
        }
        else if (pilihan == "10"){
            ...
            peekData(riwayat, sTop, antrian, qCount);
        }
        else if (pilihan == "12"){
